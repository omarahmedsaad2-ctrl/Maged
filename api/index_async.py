...
# --- Bot Logic ---
async def ask_ollama(system_prompt, user_message):
...
async def await send_telegram(chat_id, text):
    for i in range(0, len(text), 4000):
        try:
            r = await async_client.post(f"{TELEGRAM_API}/sendMessage",
...
async def await send_telegram_typing(chat_id):
    try:
        await async_client.post(f"{TELEGRAM_API}/sendChatAction",
            json={"chat_id": chat_id, "action": "typing"}, timeout=15)
    except:
        pass

async def await send_telegram_keyboard(chat_id, text, keyboard):
    try:
        r = await async_client.post(f"{TELEGRAM_API}/sendMessage",
...
async def mark_whatsapp_read(message_id):
    ...
    try:
        ...
        await async_client.post(url, headers=headers, json={
...
async def send_whatsapp(to_phone, text):
    ...
    for i in range(0, len(text), 4000):
        ...
        try:
            r = await async_client.post(url, headers=headers, json=data, timeout=60)
